...
import {
  EmojiWallDisplay,
  type EmojiWallEffect,
  type GravityRemovalMode,
...
export default function WallEmoteOverlay({ overlayId }: WallEmoteOverlayProps) {
  ...
  const fireworkRadius =
    settings?.fireworkExplosionRadius ?? settings?.fireworkRadius ?? 200;
  ...
  useEffect(() => {
    ...
    minEmojiLifetime,
    maxEmojiLifetime,
    minGravityBounceCount,
    maxGravityBounceCount,
    maxEmojis,
  ]);
